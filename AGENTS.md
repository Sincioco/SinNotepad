...
- Native standard Windows title bar, displaying `Sin - Notepad - <full path>` or the numbered name before a file has a path.
...
- Text zoom, logical line numbers in the editor gutter, and status-bar line/column information. Provide a persistent View → Line numbers toggle; numbers must remain visible while typing when enabled.
